...
export const GameService = {
    ...
    async getLeaderboardFallback(limit = 10, offset = 0) {
        ...
        // Filter for unique players and stop when we have enough for the requested page
        const topPlayers = [];
        const seenPlayers = new Set();

        for (const game of games || []) {
            if (topPlayers.length >= (offset + limit)) break;
        ...
        const pagePlayers = topPlayers.slice(offset, offset + limit);
        ...
        return {
            ...
            pagination: {
                ...
                limit,
                hasMore: false,
                hasNext: (topPlayers.length >= (offset + limit))
            }
        };
    },
